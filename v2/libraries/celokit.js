...
const transfercGOLD = async (senderId, recipientId, amount) => {
    try{
...
const getTxAmountFromHash = async (hash) => {
...
async function weiToDecimal(valueInWei){
...
async function decimaltoWei(valueInDecimal){
...
const approveTransferFrom = async (approvedAddress, approvedAmount, address, privatekey) => {
  try{
    kit.setFeeCurrency(contractkit.CeloContract.StableToken);
    kit.addAccount(privatekey);
...
const sendUBIClaim = async (sender, privatekey, UBISCADDRESS) => {
...
const checkIfBeneficiary = async (sender, UBISCADDRESS) => {
  try {
      let ubiContract = new web3.eth.Contract(
          impactUBI,
          UBISCADDRESS
...
const addBeneficiary = async (manager, _beneficiary, privatekey, UBISCADDRESS) => {
...
const checkIfManager = async (signer, UBISCADDRESS) => {
  try {
      let ubiContract = new web3.eth.Contract(
          impactUBI,
          UBISCADDRESS
...
const checkUbiScBalance = async (UBISCADDRESS) => {
  try {
...
const checkClaimCountdown = async (sender) => {
  try {
      let ubiContract = new web3.eth.Contract(
          impactUBI,
          UBISCADDRESS
...
const sendCELO = async(sender, receiver, amount, privatekey) => {
    kit.addAccount(privatekey)
    const celotoken = await kit.contracts.getGoldToken()
    const balance = await celotoken.balanceOf(sender)
...
const validateWithdrawHash = async (hash, escrowAddress) => {
  try{
...
const getWithdrawerBalance = async (publicAddress) => {
  const cusdtoken = await kit.contracts.getStableToken();
